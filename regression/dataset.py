...
class Dataset(object):
    ...
    def load_annotations(self):
        ...
        self.reg_num = len([int(res) for res in line.split(' ')[1].split(',')])
    ...
    def parse_annotations(self, annotations, dataaug,idx):
        ...
        try:
            ...
            label = [int(res) for res in line.split(' ')[1].split(',')]
            #print label
        ...
        if dataaug == True:
            try:
                ...
                if 'crop' in self.augmentation_list:
                    image = self.random_crop(image, 0.8, 0.1)
                if 'translation' in self.augmentation_list:
                    image = self.image_translation(image)
            except Exception as e:
                print (e, img_path)
        try:
            image, scale = self.img_resize(image, (cfg.TRAIN.INPUTSIZE, cfg.TRAIN.INPUTSIZE))
        except Exception as e:
            print (e, img_path)
    ...
    def next(self):
        if self.batch_count < self.num_batches:
            ...
            batch_label = np.zeros((self.batchsize, self.reg_num), dtype=np.float32)
            batch_scale = np.zeros((self.batchsize, 1), dtype=np.float32)
            ...
            for num, img in enumerate(self.image_batch):
                image, label, scale = img[0], img[1], img[2]
                batch_image[num, :, :, :] = image
                batch_scale[num, :] = scale
                for idx in range(self.reg_num):
                    batch_label[num, idx] = float(label[idx])

            self.batch_count += 1
            return batch_image, batch_label, batch_scale
